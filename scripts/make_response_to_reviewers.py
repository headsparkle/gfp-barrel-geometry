...
response('The reviewer’s size intuition is reasonable, and we tested it directly. At '
         'matched resolution (mean 1.86 vs 1.84 Å), the two classes differ in which '
         'partner is better ordered. In green FPs the compact, deeply buried HBI '
         'chromophore is more ordered than its barrel (mean B-factor ratio 0.76; '
         'absolute chromophore B 22.6 Å² vs barrel 28.6 Å²). In red FPs the '
         'chromophore’s own absolute B-factor is higher (27.7 Å²) and essentially '
         'matches its barrel (26.8 Å²; ratio 1.02); because resolution is matched, '
         'this is not a refinement artifact. To test whether the larger chromophore '
         'is simply floppy at its acylimine extension, we computed per-atom '
         'B-factors for the 64 acylimine-type (NRQ/CRQ) red chromophores '
         '(scripts/analyze_red_chromophore_bfactors.py): normalized to the barrel, '
         'the acylimine backbone (1.16), the residue-1 side chain (1.17), and the '
         'conjugated imidazolinone/phenol core (1.15) are statistically '
         'indistinguishable (paired Wilcoxon p = 0.8 and 0.4). The elevated mobility '
         'is therefore distributed uniformly across the red chromophore, not '
         'localized to a flexible extension. We conclude that the whole red '
         'chromophore is held less rigidly relative to its barrel than the smaller, '
         'deeply buried green HBI, and — because this is a crystallographic B-factor '
         '(ordering) measure — we do not infer a specific dynamical mechanism. We '
         'have revised the B-factor subsection accordingly.')
